...
目标是把未分类内容归位，并校准已经结构化但陈旧、重复、错误或残留模板的记忆。

## 执行准备

1. 确认目标 Git 仓库和分支。默认使用当前仓库与当前分支；用户指定时使用指定值。
2. 确认 `dev-memory-cli` 可用。若正在 dev-memory-skill-suite 源码仓内开发，可使用 `node <repo>/bin/dev-memory.js`；否则使用已安装的 `dev-memory-cli`。
3. 后续所有命令都显式传 `--repo <REPO>`，指定了非当前分支时再传 `--branch <BRANCH>`。
4. 整理范围默认是 `branch`；只有用户明确需要同时校准 repo 共享层时才使用 `branch+repo`。

以下用 `<CLI>` 表示上一步确认的 CLI 命令。

## 硬规则

- 始终只处理已确认的单个 repo + branch。
...
`SCOPE` 使用已确认的 `branch` 或 `branch+repo`。第一次 prepare 后优先读取输出中的 `annotated_md`，不要只看 stdout 的扁平 entries。它保留了：
...
把相关条目聚合成约 3～8 个 proposal，每个 proposal 包含 `id`、`priority`、`title`、`reason`、`actions`。优先使用粗粒度动作：
...
apply 会先备份，再按 reset → section → block → entry 的顺序执行。完成后读取生成的 `summary_*.md`，核对 accepted/rejected/custom、实际 rewritten/deleted 和 invalid actions。
